...
function IonConcentration(ion::IonSpecies, conc::Union{Nothing, Real, Molarity} = nothing;
    ...
    if isnothing(conc_val)
        ret = dynamic ? only(@variables $sym(t) [unit = µM]) :
              only(@parameters $sym [unit = µM])
    else
...
function IonCurrent(ion::IonSpecies, curr::Union{Nothing, Real, Current} = nothing;
    ...
    if isnothing(curr_val)
        ret = dynamic ? only(@variables $name(t) [unit = µA]) :
              only(@parameters $name [unit = µA])
    else
...
function EquilibriumPotential(ion::IonSpecies, eqv::Union{Nothing, Real, Voltage};
    ...
    if isnothing(eqv_val)
        ret = dynamic ? only(@variables $sym(t) [unit = mV]) :
              only(@parameters $sym [unit = mV])
    else
...
function Equilibria(equil::Vector)
    out = Num[]
    for x in equil
        x.first isa IonSpecies ||
            throw("Equilibrium potential must be associated with an ion type.")
